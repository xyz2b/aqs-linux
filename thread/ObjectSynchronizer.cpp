...
void ObjectSynchronizer::slow_enter(InstanceOopDesc *obj, BasicLock* lock, JavaThread *t) {
    ...
    // 只有无锁才能变成轻量级锁
    // 如果在持有轻量级锁的线程释锁之前，还未有线程来抢锁，即还未膨胀，那么在其释放轻量级锁之后，别的线程还可以来抢轻量级，这个不需要特别做什么逻辑
    //       1.因为轻量级锁在解锁时，会将加轻量级锁之前的对象头保存起来（无锁的对象头），然后解锁时再设置回来，如果没有膨胀，那么解锁完成之后，对象头是无锁，别的线程还可以加轻量级锁
    //       2.在膨胀成重量级锁时，对象头会先被改为00中间状态，之后被改为重量级锁（即使重量级锁被释放也不会将对象头改回），所以别的线程再抢轻量级锁时，对象头不会是无锁状态，加不了轻量级锁，只有膨胀
    // 如果已经膨胀成重量级锁了，之前轻量级锁的线程释放了轻量级锁，别的线程也获取不到轻量级锁了，回退不了
    if(mark->is_neutral()) {
    ...
    for (;;) {
        if ((bool)(Atomic::cmpxchg_ptr((void*)true, (void *) &(monitor->_entryListLock), (void*)false)) == false) {
            ...
            if (monitor->_entryList == NULL) {
                monitor->_entryList = node;
            } else {
                node->_next = monitor->_entryList;
                monitor->_entryList->_prev = node;
                monitor->_entryList = node;
            }
            monitor->_entryListLength++;
            monitor->_entryListLock = false;
            break;
        } else {
            INFO_PRINT("[%s] 加入waiter set失败", t->_name.c_str());
